...
const colorVisitedNodeConnections = (node1, node2) =>{
    ...
    ctxArrow.strokeStyle = 'red';
        if (matrix[i][j] == 1 && !queue.contains(i)) {
            if(i != j)
            {
                ctxArrow.beginPath();
                ctxArrow.moveTo(nodePositions[j].x, nodePositions[j].y);
...
let visitedINdex = 0;
const nextStep = () =>{
    let connected = connectedNodes(queue.storage[queue.head]);
    visitedNodes.enqueue(queue.storage[queue.head]);
    queue.dequeue();
    for(let q of connected)
    {
        if(!visitedNodes.contains(q) && !queue.contains(q))
        {
            queue.enqueue(q);
            colorQueueNode(q);
            colorQueueNodeConnections(visitedNodes.storage[visitedINdex], q);
            if(matrix[q][q] === 1)
            colorQueueNodeConnections(q, q);
            nodesAndParents[q] = visitedNodes.storage[visitedINdex];
        }
    }
    colorVisitedNode(visitedNodes.storage[visitedINdex]);
    console.log(nodesAndParents)
    console.log(visitedNodes.storage[visitedINdex]);
    console.log(nodesAndParents[visitedNodes.storage[visitedINdex]]);
    if(visitedINdex > 0)
    colorVisitedNodeConnections(nodesAndParents[visitedNodes.storage[visitedINdex]], visitedNodes.storage[visitedINdex]);
    Object.values(visitedNodes.storage).forEach(function(value) {
